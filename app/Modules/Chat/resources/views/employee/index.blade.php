...
<div class="row">
        ...
        <div class="panel panel-default" style="border-radius: 12px; border: 1px solid #eef0f2; box-shadow: 0 4px 15px rgba(0,0,0,0.03); overflow: hidden;">
            <div class="panel-heading" style="background-color: white; border-bottom: 1px solid #eee; padding: 15px;">
                <h4 style="margin: 0; font-weight: 700; color: #1e3c72; font-size: 16px;">
                    <i class="fa fa-comments" style="margin-right: 5px;"></i> Messages from HR
                </h4>
            </div>
            
            <div class="panel-body" style="padding: 10px;">
                <!-- Conversation List Quick Filter Search -->
                <div class="form-group" style="margin-bottom: 10px;">
                    <div class="input-group">
                        ...
                        <input type="text" id="conversation-search" class="form-control" placeholder="Search threads..." style="border-left: none; border-radius: 0 8px 8px 0; height: 38px; box-shadow: none;">
                    </div>
                </div>
                ...
                <div class="list-group" id="conversation-list" style="max-height: 480px; overflow-y: auto; margin-bottom: 0;">
                    @forelse($conversations as $conv)
                        <?php
                            $hrManager = $conv->hrManager ?: $conv->creator;
                        ...
                        ?>
                        <a href="{{ route('employee.chat.index', ['conversation_id' => $conv->id]) }}" 
                           class="list-group-item conversation-item {{ $isActive ? 'active-chat' : '' }}" 
                           data-subject="{{ strtolower($conv->subject) }}"
                           data-sender="{{ strtolower($hrManager ? $hrManager->first_name . ' ' . $hrManager->last_name : '') }}"
                            ...
                            <div style="display: flex; align-items: center; justify-content: space-between;">
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <div style="width: 38px; height: 38px; border-radius: 50%; background-color: {{ $isActive ? '#1e3c72' : '#e9ecef' }}; color: {{ $isActive ? 'white' : '#495057' }}; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 13px;">
                                        @if($hrManager)
                                            {{ substr($hrManager->first_name, 0, 1) }}{{ substr($hrManager->last_name, 0, 1) }}
                                        @else
                                            HR
                                        @endif
                                    </div>
                                    ...
                                    <div>
                                        <h5 style="margin: 0; font-weight: 700; color: #333; font-size: 14px;">
                                            {{ $hrManager ? $hrManager->first_name . ' ' . $hrManager->last_name : 'HR Team' }}
                                        </h5>
                    ...
                    @empty
                        <div class="text-center text-muted" style="padding: 30px 10px;">
                            ...
                            <p style="font-size: 13px;">No messages from HR yet.</p>
                        </div>
                    @endforelse
                </div>
            </div>
    ...
    <div class="col-md-8">
        @if($activeConversation)
            <?php $hrManager = $activeConversation->hrManager ?: $activeConversation->creator; ?>
            <div class="panel panel-default" style="border-radius: 12px; border: 1px solid #eef0f2; box-shadow: 0 4px 15px rgba(0,0,0,0.03); overflow: hidden;">
                <!-- Conversation Header -->
                <div class="panel-heading" style="background-color: white; border-bottom: 1px solid #eee; padding: 15px;">
                    <div style="display: flex; align-items: center; justify-content: space-between;">
                        <div style="display: flex; align-items: center; gap: 10px;">
                            ...
                            <div>
                                <h4 style="margin: 0; font-weight: 700; color: #1e3c72; font-size: 16px;">
                                    {{ $hrManager ? $hrManager->first_name . ' ' . $hrManager->last_name : 'HR Manager' }}
                                </h4>
                                <p style="margin: 2px 0 0 0; font-size: 11px; color: #888;">
                                    Subject: <strong>{{ $activeConversation->subject }}</strong>
                                </p>
                            </div>
                        </div>
                        <span class="label label-primary">HR Conversation</span>
                    </div>
                </div>
                ...
                <div class="panel-footer" style="background-color: white; border-top: 1px solid #eee; padding: 15px;">
                    <form action="{{ route('employee.chat.reply', $activeConversation->id) }}" method="POST">
                        {{ csrf_field() }}
                        <div class="input-group">
                            <input type="text" name="body" class="form-control" placeholder="Type your response..." required style="height: 44px; border-radius: 22px 0 0 22px; border: 1px solid #ccc; padding-left: 20px; box-shadow: none;" autocomplete="off">
        ...
        @else
            ...
            <div class="panel panel-default" style="border-radius: 12px; border: 1px solid #eef0f2; box-shadow: 0 4px 15px rgba(0,0,0,0.03); overflow: hidden;">
                <div class="panel-body text-center" style="padding: 120px 20px; background-color: #fcfdfe;">
                    ...
                    <h3 style="margin-top: 0; font-weight: 700; color: #4a5568;">Your HR Inbox</h3>
                    <p class="text-muted" style="max-width: 400px; margin: 0 auto;">
                        Click on a messaging thread in the left pane to read and reply to messages from the HR Department and Admin.
                    </p>
                </div>
            </div>
        @endif
    ...
    if (convSearch) {
        convSearch.addEventListener('keyup', function() {
            ...
            items.forEach(function(item) {
                ...
                var sender = item.getAttribute('data-sender');
                if (subject.indexOf(filter) > -1 || sender.indexOf(filter) > -1) {
